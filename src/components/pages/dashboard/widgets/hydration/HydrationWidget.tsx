    ...
    >
      <div className="flex flex-col gap-[6cqw] @sm:gap-4">
        <div className="flex items-center gap-[6cqw] @sm:gap-5">
          <div className="flex-1 p-[5cqw] @sm:p-5 rounded-xl bg-blue-500/5 border border-blue-500/10">
        ...
        <div className="space-y-[2cqw] @sm:space-y-2">
          {reminders.slice(0, 2).map((r) => (
            <div
              key={`h-key${r.id}`}
              className="flex items-center gap-[4cqw] @sm:gap-3 p-[3cqw] @sm:p-3 rounded-xl bg-neutral-800/20 border border-border/40"
            >
              <Bell className="w-[4cqw] h-[4cqw] @sm:w-4.5 @sm:h-4.5 text-blue-500/60" />
              <span className="text-[3.5cqw] @sm:text-sm font-medium text-muted-foreground truncate flex-1">
