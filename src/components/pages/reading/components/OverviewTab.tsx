    ...
    <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div
            key={card.label}
            className="bg-card border border-border rounded-xl p-4 flex flex-col gap-2 hover:border-orange-500/20 transition-all"
          >
        ...
        <div className="md:col-span-8 flex flex-col gap-4">
          <div className="flex items-center justify-between px-1">
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-orange-500/10 hover:bg-orange-500/20 border border-orange-500/60 text-orange-500 text-[9px] font-bold transition-all"
            >
            ...
            {currentBooks.length > 0 ? (
              currentBooks.map((book) => {
                ...
                return (
                  <div
                    key={book.id}
                    className="bg-card/50 border border-border rounded-xl p-5 flex flex-col gap-4 group hover:bg-card transition-all border-b-2 border-b-neutral-800 hover:border-b-orange-500/30"
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-2 flex items-center gap-2">
                          <span className="text-[9px] font-semibold px-2 py-0.5 rounded-xl bg-orange-500/10 text-orange-600 dark:text-orange-400 border border-orange-500/20">
                            {book.category}
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="h-1.5 bg-neutral-800 rounded-xl overflow-hidden">
                        <div
                          className="h-full bg-orange-500 rounded-xl transition-all duration-1000"
                          style={{ width: `${progress}%` }}
                        />
                      </div>
                      <div className="flex justify-between text-[9px] font-medium text-neutral-600">
                        <span>{book.current_page} pág.</span>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between px-1">
              ...
              <button
                ...
                className="text-[9px] font-bold text-orange-500 hover:text-orange-600 dark:text-orange-400 transition-colors"
              >
                Configurar
              </button>
            </div>
            ...
            <div className="bg-card border border-border rounded-xl p-4 flex flex-col gap-5">
              {(() => {
                ...
                const weeklyGoals = targetTypes
                  .map((type) => goals.find((g) => g.goal_type === type))
                ...
                return weeklyGoals.map((goal) => {
                  const isPages = goal.goal_type === "PagesPerWeek";
                    ...
                    <div key={goal.goal_type} className="space-y-2">
                      <div className="flex justify-between items-end">
                        ...
                        <span className="text-[10px] font-bold text-foreground tabular-nums">
                          {current} / {Math.round(goal.target_value)}
                        </span>
                      </div>
                      <div className="h-1.5 bg-black/40 rounded-full overflow-hidden border border-border/50">
                        <div
                          className={cn(
                            ...
                            isPages ? "bg-orange-500" : "bg-amber-500",
                          )}
                          style={{ width: `${progress}%` }}
                        />
                      </div>
