...
export function CorrelationChart({ metrics }: CorrelationChartProps) {
  ...
  const maxStudy = Math.max(...metrics.map((m) => m.studyHours), 0.1);
  const maxSleep = Math.max(...metrics.map((m) => m.sleepHours), 0.1);
  const maxReading = Math.max(...metrics.map((m) => m.readingPages), 1);
      ...
      <div className="overflow-x-auto scrollbar-hide">
        ...
        >
          {metrics.slice(-30).map((m) => {
            const studyPct = (m.studyHours / maxStudy) * 100;
            const sleepPct = (m.sleepHours / maxSleep) * 100;
            const readingPct = (m.readingPages / maxReading) * 100;
            const hitPct = m.studyHitRate;
              ...
              >
                ...
                <div className="flex items-end gap-0.5 h-32 w-full">
                  ...
                  <div
                    ...
                    title={`Leitura: ${m.readingPages} pág.`}
                  />
                </div>
                {/* Rótulo do eixo X */}
