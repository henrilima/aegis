  ...
  return (
    <div className="fixed inset-0 z-300 flex items-center justify-center p-4">
      <ToolTip content="Fechar Busca">
        <button
          type="button"
          className="absolute inset-0 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300 border-none outline-none cursor-default w-full h-full"
          onClick={onClose}
          aria-label="Fechar modal"
        />
      </ToolTip>
      <div className="relative w-full max-w-3xl max-h-[85vh] bg-background border border-border rounded-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
        {/* Header */}
                            ...
                            <div className="flex items-start justify-between gap-4">
                              ...
                              <ToolTip content="Salvar no Glossário">
                                <button
                                  ...
                                  className={cn(
                                    "shrink-0 flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all",
                                    theme.bg,
                                    theme.text,
                                    theme.solidHover,
                                    "hover:text-white",
        ...
        <div className="px-6 py-4 bg-card border-t border-border flex justify-end">
          <ToolTip content="Fechar Resultados">
            <button
              ...
              onClick={onClose}
              className={cn(
                "px-8 py-2 rounded-xl text-white text-xs font-bold transition-all active:scale-95",
                theme.solid,
                theme.solidHover,
              )}
            >
